...
class SECOND3D(BaseModule):
    ...
    def forward(self, x):
        ...
        # for out in outs:
        #     print(out.shape)
        # torch.Size([1, 128, 5, 180, 180]) 
        # torch.Size([1, 256, 5, 90, 90])  
        # torch.Size([1, 512, 5, 45, 45])   

        outs_bind = outs[0]
        for i in range(len(self.blocks) - 1):
            outs_bind = torch.cat([outs_bind, outs[i + 1]], dim=1)

        return {'x': outs_bind.permute(0,1,4,3,2), # not dense:  B, C, W, H, D 
                'pts_feats': [outs_bind]}
