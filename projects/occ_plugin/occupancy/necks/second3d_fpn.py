...
class SECOND3DFPN(BaseModule):
    ...
    def __init__(self,
        ...
        # 转换结构
        self.change_output = build_conv_layer(dict(type='Conv3d'),
                                              out_channels[-1],
                                              out_channels[-1],
                                              (1,1,1)
                                              )
        
        self.change_output2 = build_conv_layer(
            dict(type='Conv2d'),
            5 * 256,
            512,
            (1,1)
        ...
        # self.change_output3 = nn.Linear(128*128, 180*180)
    ...
    def forward(self, x):
        ...
        out = self.change_output(out)
        out = out.view(out.size()[0], -1, out.size()[3], out.size()[4])
        out = self.change_output2(out)
        # out = out.view(out.size()[0], out.size()[1], -1)
        # out = self.change_output3(out)
        # out = out.view(out.size()[0], out.size()[1], 180, 180)
